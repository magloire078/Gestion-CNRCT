        ...
        <form onSubmit={handleSubmit} className="flex flex-col h-full bg-white">
          <SheetHeader className="p-6 bg-slate-950 text-white shrink-0 text-left relative overflow-hidden">
            <div className="absolute top-[-20%] right-[-10%] p-5 opacity-20 rotate-12">
               <svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-500"><path d="M12 2v20"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            </div>
            ...
            <div className="relative z-10 space-y-4">
               <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-emerald-500/20 backdrop-blur-xl border border-emerald-500/30 flex items-center justify-center text-emerald-400 shadow-lg shadow-emerald-500/10">
                  ...
                  <div>
                     <SheetTitle className="text-2xl font-black tracking-tight text-white uppercase italic leading-none">Édition Budgétaire</SheetTitle>
               ...
               <SheetDescription className="text-slate-400 font-bold italic text-sm">
                 Modification de la ligne budgétaire pour l'exercice {year}
               </SheetDescription>
            </div>
          </SheetHeader>
          ...
          <div className="flex-1 px-6 py-5 space-y-5 overflow-y-auto custom-scrollbar bg-slate-50/30">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-3 group">
                  <Label htmlFor="type-edit" className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1 transition-colors group-focus-within:text-emerald-600">Nature de la Ligne</Label>
                  <Select value={type} onValueChange={(val: 'emploi' | 'ressource') => setType(val)}>
                    <SelectTrigger id="type-edit" className="h-14 border-slate-200 bg-white rounded-2xl focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all font-black text-slate-700 shadow-sm">
               ...
               <div className="space-y-3 group">
                  <Label htmlFor="code-edit" className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1 transition-colors group-focus-within:text-emerald-600">Ligne Budgétaire (Code)</Label>
                  <Input 
                    id="code-edit" 
                    placeholder="ex: 601"
                    value={code} 
                    onChange={(e) => setCode(e.target.value)} 
                    className="h-14 border-slate-200 bg-white rounded-2xl focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all font-black text-xl px-5 shadow-sm text-slate-900"
                  />
               </div>
            </div>
            ...
            {/* Section Libellé */}
            <div className="space-y-3 group">
              <Label htmlFor="name-edit" className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1 transition-colors group-focus-within:text-emerald-600">Désignation de la Ligne</Label>
              <Input 
                id="name-edit" 
                placeholder="Libellé complet du poste budgétaire"
                value={name} 
                onChange={(e) => setName(e.target.value)} 
                className="h-14 border-slate-200 bg-white rounded-2xl focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all font-bold px-5 shadow-sm text-slate-900"
              />
            </div>
            ...
            <div className="p-5 bg-slate-950 rounded-xl space-y-4 shadow-[0_20px_50px_rgba(0,0,0,0.2)] relative overflow-hidden group/card">
               ...
               <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 rounded-bl-full transition-all group-hover/card:scale-110" />
               ...
               <div className="relative z-10 flex items-center gap-4">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Ventilation Financière</span>
               </div>
               
               <div className="relative z-10 grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <Label htmlFor="amount-edit" className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-2">Dotation Exercice N ({year})</Label>
                    <div className="relative group/input">
                      <Input 
                        ...
                        className="h-16 border-white/5 bg-white/5 text-white rounded-[1.25rem] focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500/50 transition-all font-black text-2xl pl-6 pr-14"
                      />
                      <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] font-black text-emerald-500/40 tracking-tighter">CFA</span>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <Label htmlFor="prev-amount-edit" className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-2">Dotation Exercice N-1</Label>
          ...
          <SheetFooter className="p-6 border-t border-slate-100 bg-white flex flex-row items-center justify-end gap-4 shrink-0">
            ...
            <Button 
              ...
              className="h-14 px-12 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] bg-slate-950 hover:bg-slate-900 text-white shadow-2xl shadow-slate-300 transition-all active:scale-95 hover:shadow-emerald-500/10"
            >
              {isSubmitting ? (
                <div className="flex items-center gap-3">
