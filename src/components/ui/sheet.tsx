...
>(({ side, className, children, ...props }, ref) => {
    // Default to left on desktop, bottom on mobile
    const defaultSide = typeof window !== 'undefined' && window.innerWidth < 768 ? 'bottom' : 'left';
    ...
    return (
      <SheetPortal>
        ...
        <SheetPrimitive.Content
          ref={ref}
          className={cn(sheetVariants({ side: finalSide }), "p-0", className)}
          {...props}
        >
          {children}
...
const SheetFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 mt-auto p-6 border-t",
      className
    )}
    {...props}
  />
