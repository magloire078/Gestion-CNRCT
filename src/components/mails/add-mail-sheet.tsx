...
interface AddMailSheetProps {
  isOpen: boolean;
  onCloseAction: () => void;
  onAddMailAction: (mail: Omit<Mail, "id" | "trackingId">) => Promise<void>;
}
      ...
      <SheetContent className="w-full sm:max-w-xl p-0 flex flex-col h-full bg-slate-50">
        <div className="p-6 bg-white border-b border-slate-100 flex items-center justify-between shrink-0">
          <SheetHeader className="space-y-1">
            <SheetTitle className="text-xl font-bold tracking-tight text-slate-900">Enregistrer un Courrier</SheetTitle>
            <SheetDescription className="text-xs text-slate-500">
              Remplissez le formulaire ci-dessous pour ajouter un nouveau courrier au registre.
            </SheetDescription>
          </SheetHeader>
        </div>

        <ScrollArea className="flex-1 px-6 py-4">
          <form onSubmit={handleSubmit} id="add-mail-form" className="space-y-5 pb-6">
            {error && (
        ...
        <div className="p-4 bg-white border-t border-slate-100 flex items-center justify-end gap-3 shrink-0">
          <SheetClose asChild>
            <Button variant="outline" className="border-slate-200 font-semibold text-xs" onClick={resetForm} disabled={isSubmitting}>
              Annuler
            </Button>
          </SheetClose>
          <Button
