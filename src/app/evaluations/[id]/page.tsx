    ...
    useEffect(() => {
        ...
        async function fetchEvaluation() {
            try {
                ...
                // Ensure goals array exists and each goal has an ID
                if (data && data.goals) {
             ...
             <Card>
                ...
                <CardContent className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <Textarea id="strengths" value={evaluation.strengths} onChange={(e) => handleCommentChange(e, 'strengths')} rows={4} placeholder="Décrire les points forts de l'employé..."/>
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="areasForImprovement">Axes d'Amélioration</Label>
                        <Textarea id="areasForImprovement" value={evaluation.areasForImprovement} onChange={(e) => handleCommentChange(e, 'areasForImprovement')} rows={4} placeholder="Identifier les domaines à améliorer..."/>
                    </div>
                     <div className="space-y-2">
                        <Label htmlFor="managerComments">Commentaires du Manager</Label>
                        <Textarea id="managerComments" value={evaluation.managerComments} onChange={(e) => handleCommentChange(e, 'managerComments')} rows={4} placeholder="Ajouter des commentaires généraux..."/>
                    </div>
                     <div className="space-y-2">
                        <Label htmlFor="employeeComments">Commentaires de l'Employé</Label>
                        <Textarea id="employeeComments" value={evaluation.employeeComments || ''} onChange={(e) => handleCommentChange(e, 'employeeComments')} rows={4} placeholder="L'employé peut ajouter ses commentaires ici..."/>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                 <CardHeader>
                    <div className="flex justify-between items-center">
                        ...
                        <Button variant="outline" size="sm" onClick={addGoal}><PlusCircle className="mr-2 h-4 w-4"/> Ajouter</Button>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4">
                             ...
                             <div key={goal.id} className="p-4 border rounded-lg space-y-3">
                                <div className="flex justify-between items-start gap-2">
                                     <div className="flex-1 space-y-1">
                                        ...
                                        <Input id={`goal-title-${goal.id}`} value={goal.title} onChange={(e) => handleGoalChange(goal.id, 'title', e.target.value)} placeholder="Titre de l'objectif"/>
                                    </div>
                                    <div className="space-y-1 w-[180px]">
                                        <Label htmlFor={`goal-status-${goal.id}`}>Statut</Label>
                                        <Select value={goal.status} onValueChange={(value: Goal['status']) => handleGoalChange(goal.id, 'status', value)}>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Textarea id={`goal-desc-${goal.id}`} value={goal.description} onChange={(e) => handleGoalChange(goal.id, 'description', e.target.value)} placeholder="Description..." rows={2}/>
                                </div>
