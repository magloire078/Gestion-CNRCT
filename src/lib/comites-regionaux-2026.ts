...
export const comitesRegionaux2026List: ComiteRegionalMember[] = rawData
  .trim()
  .split('\n')
  .map(line => {
    ...
    const nom = parts[3] || '';
    const prenoms = parts[4] || '';
    const nomComplet = `${nom} ${prenoms}`.replace(/\s+/g, ' ').trim();
    const fonctionLocalite = parts[5] || '';
    const contacts = parts[6] || '';
...
export function findComiteRegionalMember(nameOrFullName: string, region?: string, department?: string): ComiteRegionalMember | undefined {
  ...
  if (comitesByNormName.has(norm)) {
    return comitesByNormName.get(norm);
  }
  ...
  for (const item of comitesRegionaux2026List) {
