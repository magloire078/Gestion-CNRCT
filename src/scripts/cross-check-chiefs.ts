...
async function main() {
  ...
  console.log(`Parsed ${provided.length} provided entries.`);

  console.log('Fetching Firestore collections: chiefs, employees...');
  const chiefsSnap = await db.collection('chiefs').get();
  const employeesSnap = await db.collection('employees').get();

  const dbChiefs = chiefsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));
  const dbEmployees = employeesSnap.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));

  console.log(`Firestore chiefs: ${dbChiefs.length}`);
  console.log(`Firestore employees: ${dbEmployees.length}`);

  // We will match each provided entry against dbChiefs
  const results: any[] = [];
  const matchedChiefIds = new Set<string>();

  for (const entry of provided) {
    if (!entry.nom && !entry.prenoms) {
      results.push({
        status: 'EMPTY_ROW',
        entry,
        match: null,
        notes: `Ligne #${entry.num} dans la liste fournie est incomplète (sans nom/prénom) : Région ${entry.region}, Dépt ${entry.department}`,
      });
      continue;
    }

    const entryFullName = `${entry.nom} ${entry.prenoms}`;
    const normEntryName = normalize(entryFullName);
    const normEntryNom = normalize(entry.nom);
    const normEntryPrenoms = normalize(entry.prenoms);
    const normEntryPhone = normalizePhone(entry.contacts);

    // Try finding exact or high similarity match
    let bestMatch: any = null;
    let matchScore = 0;
    let matchReason = '';

    for (const c of dbChiefs) {
      const cFullName = `${c.lastName || c.nom || ''} ${c.firstName || c.prenoms || ''}`.trim() || c.name || '';
      const normCName = normalize(cFullName);
      const normCLastName = normalize(c.lastName || c.nom || '');
      const normCFirstName = normalize(c.firstName || c.prenoms || '');
      const normCPhone = normalizePhone(c.phone || c.contact || c.contacts || '');

      let score = 0;
      let reasons: string[] = [];

      // Exact full name match
      if (normEntryName && normCName && (normEntryName === normCName || normCName.includes(normEntryName) || normEntryName.includes(normCName))) {
        score += 60;
        reasons.push('Nom complet similaire');
      } else if (normEntryNom && normCLastName && normEntryNom === normCLastName) {
        score += 30;
        reasons.push('Nom de famille identique');
        if (normEntryPrenoms && normCFirstName && (normCFirstName.includes(normEntryPrenoms) || normEntryPrenoms.includes(normCFirstName))) {
          score += 25;
          reasons.push('Prénoms partiels correspondants');
        }
      }

      // Phone match
      if (normEntryPhone && normCPhone && (normEntryPhone === normCPhone || normCPhone.endsWith(normEntryPhone) || normEntryPhone.endsWith(normCPhone))) {
        score += 40;
        reasons.push('Numéro de téléphone identique');
      }

      // Locality / Department / Village match
      const normRegion = normalize(c.region || '');
      const normDept = normalize(c.department || '');
      const normVillage = normalize(c.village || '');

      if (normRegion && normalize(entry.region).includes(normRegion) || normRegion.includes(normalize(entry.region))) {
        score += 10;
      }
      if (normDept && normalize(entry.department).includes(normDept) || normDept.includes(normalize(entry.department))) {
        score += 10;
      }
      if (normVillage && normalize(entry.fonctionLocalite).includes(normVillage)) {
        score += 15;
        reasons.push(`Village (${c.village}) correspond`);
      }
    ...
    if (bestMatch) {
      matchedChiefIds.add(bestMatch.id);

      // Check differences
      const diffs: string[] = [];
      const dbPhone = bestMatch.phone || bestMatch.contact || bestMatch.contacts || '';
      if (normEntryPhone && normalizePhone(dbPhone) !== normEntryPhone) {
        diffs.push(`Téléphone : DB="${dbPhone}" vs Fourni="${entry.contacts}"`);
      }
      if (entry.region && normalize(bestMatch.region) !== normalize(entry.region)) {
        diffs.push(`Région : DB="${bestMatch.region}" vs Fourni="${entry.region}"`);
      }
      if (entry.department && normalize(bestMatch.department) !== normalize(entry.department)) {
        diffs.push(`Département : DB="${bestMatch.department}" vs Fourni="${entry.department}"`);
      }

      results.push({
        status: diffs.length > 0 ? 'MATCH_WITH_DIFFS' : 'EXACT_MATCH',
        entry,
        match: bestMatch,
        score: matchScore,
        matchReason,
  ...
  // Find DB chiefs that were NOT in the provided list
  const unmatchedDbChiefs = dbChiefs.filter(c => !matchedChiefIds.has(c.id));

  // Write detailed report to JSON & console
  const summary = {
    totalProvided: provided.length,
    emptyRows: results.filter(r => r.status === 'EMPTY_ROW').length,
    exactMatches: results.filter(r => r.status === 'EXACT_MATCH').length,
    matchesWithDiffs: results.filter(r => r.status === 'MATCH_WITH_DIFFS').length,
    notFoundInDb: results.filter(r => r.status === 'NOT_FOUND_IN_DB').length,
    unmatchedInDb: unmatchedDbChiefs.length,
  };

  const report = {
    summary,
    results,
    unmatchedDbChiefs: unmatchedDbChiefs.map(c => ({
      id: c.id,
      name: `${c.lastName || c.nom || ''} ${c.firstName || c.prenoms || ''}`.trim() || c.name,
      region: c.region,
  ...
  fs.writeFileSync(path.resolve(process.cwd(), 'cross_check_report.json'), JSON.stringify(report, null, 2));
  console.log('\n=== SUMMARY ===');
  console.log(summary);
}
