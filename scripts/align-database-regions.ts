...
async function alignRegions() {
    try {
        ...
        const deptMap: Record<string, { id: string, name: string, regionId: string }> = {};
        ...
        const regionMap: Record<string, { id: string, name: string, districtId: string }> = {};
        ...
        for (const doc of villagesSnapshot.docs) {
            ...
            if (needsUpdate) {
                ...
                await db.collection('villages').doc(docId).update(updateFields);
                villageUpdates++;
            }
        }
        ...
        for (const doc of chiefsSnapshot.docs) {
            ...
            if (needsUpdate) {
                ...
                await db.collection('chiefs').doc(docId).update(updateFields);
                chiefUpdates++;
            }
        }
