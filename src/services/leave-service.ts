...
export async function addLeave(leaveDataToAdd: Omit<Leave, 'id' | 'status'>): Promise<Leave> {
    ...
    try {
        ...
        return { id: docRef.id, ...newLeaveData } as Leave;
    } catch (error: any) {
        if (error.code === 'permission-denied') {
...
export async function updateLeaveStatus(id: string, status: 'Approuvé' | 'Rejeté'): Promise<void> {
    ...
    try {
        ...
        const employeeUserId = employeeId ? await findUserIdByEmployeeId(employeeId) : null;
        ...
        if (employeeUserId) {
            await createNotification({
                ...
                href: '/leave' // Link to their leave page (filtered to them)
            });
        } else {
