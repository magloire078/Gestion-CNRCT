...
interface CrossResult {
  ...
  oldEntry?: OldEntry;
  newEntry?: NewEntry;
  type: 'RECONDUIT_IDENTIQUE' | 'RECONDUIT_AVEC_INFOS' | 'REMPLACEMENT_NOUVEAU' | 'POURVU_NOUVEAU' | 'POSTE_VACANT' | 'DECALAGE_NUMERO';
  details: string;
  ancienTitulaire: string;
  nouveauTitulaire: string;
  ancienObservation: string;
  nouveauProfile: string;
  nouveauContact: string;
  nouveauFonction: string;
}
...
for (let i = 0; i < newEntries.length; i++) {
  ...
  // Match with old by num or locality
  let old = oldEntries.find(o => o.num === n.num);

  const normNewName = normalize(n.nomComplet);
  const normOldName = old ? normalize(old.nomPrenoms) : '';
  ...
  if (!n.nomComplet) {
    comparison.push({
      ...
      newEntry: n,
      type: 'POSTE_VACANT',
      details: 'Poste toujours non pourvu / vide dans la nouvelle liste',
      ancienTitulaire: old ? old.nomPrenoms : 'Inconnu',
      nouveauTitulaire: '(VIDE / A POURVOIR)',
      ancienObservation: old ? old.observations : '',
      nouveauProfile: '',
      nouveauContact: '',
      nouveauFonction: '',
    });
  ...
  const isSame = normOldName && (normOldName === normNewName || normOldName.includes(normNewName) || normNewName.includes(normOldName) || (normalize(n.nom) && normOldName.includes(normalize(n.nom))));
  ...
  if (isSame) {
    comparison.push({
      ...
      oldEntry: old,
      newEntry: n,
      type: 'RECONDUIT_AVEC_INFOS',
      details: `Reconduit - Coordonnées & fonctions précisées (Contact: ${n.contacts || 'N/A'}, Fonction: ${n.fonctionLocalite})`,
      ancienTitulaire: old ? old.nomPrenoms : '',
      nouveauTitulaire: n.nomComplet,
      ancienObservation: old ? old.observations : '',
      nouveauProfile: n.profile,
      nouveauContact: n.contacts,
      nouveauFonction: n.fonctionLocalite,
    });
  } else {
    // Replacement or new assignment
    const wasVacantOrDeceased = old && (old.observations.toLowerCase().includes('décédé') || old.observations.toLowerCase().includes('pourvoir') || old.nomPrenoms.toUpperCase().includes('NON POURVU'));
    ...
    comparison.push({
      ...
      oldEntry: old,
      newEntry: n,
      type: wasVacantOrDeceased ? 'POURVU_NOUVEAU' : 'REMPLACEMENT_NOUVEAU',
      details: wasVacantOrDeceased 
        ? `Nouveau titulaire désigné pour un poste précédemment vacant/décédé (${old?.observations || 'A pourvoir'})`
        : `Changement de titulaire : Remplacement de "${old ? old.nomPrenoms : 'N/A'}" par "${n.nomComplet}"`,
      ancienTitulaire: old ? old.nomPrenoms : '(Aucun)',
      nouveauTitulaire: n.nomComplet,
      ancienObservation: old ? old.observations : '',
      nouveauProfile: n.profile,
      nouveauContact: n.contacts,
...
const stats = {
  ...
  nouveauxProfil: newEntries.filter(n => n.profile.toLowerCase() === 'nouveau').length,
  reconduitsProfil: newEntries.filter(n => n.profile.toLowerCase() === 'reconduit').length,
  postesVacants: comparison.filter(c => c.type === 'POSTE_VACANT').length,
  pourvusNouveau: comparison.filter(c => c.type === 'POURVU_NOUVEAU').length,
  remplacements: comparison.filter(c => c.type === 'REMPLACEMENT_NOUVEAU').length,
  reconduitsEnrichis: comparison.filter(c => c.type === 'RECONDUIT_AVEC_INFOS').length,
};

fs.writeFileSync(
  path.resolve(process.cwd(), 'croisement_resultats.json'),
  JSON.stringify({ stats, comparison }, null, 2)
);

console.log('=== STATS DU CROISEMENT ===');
console.log(stats);

console.log('\n=== DETAIL DES CHANGEMENTS / NOUVEAUX TITULAIRES / ECARTS ===');
comparison.filter(c => c.type !== 'RECONDUIT_AVEC_INFOS').forEach(c => {
  console.log(`[#${c.num}] ${c.region} - ${c.department} | ${c.type}`);
  console.log(`  Ancien : ${c.ancienTitulaire} (${c.ancienObservation})`);
  console.log(`  Nouveau : ${c.nouveauTitulaire} | Profil: ${c.nouveauProfile} | Contact: ${c.nouveauContact}`);
  console.log(`  Fonction : ${c.nouveauFonction}`);
  console.log('---');
});
