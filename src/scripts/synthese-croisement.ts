...
const oldTsvPath = path.resolve(process.cwd(), 'src/scripts/data/comites_regionaux_brut.tsv');
const oldTsvRaw = fs.readFileSync(oldTsvPath, 'utf8');
...
const oldEntries: OldEntry[] = oldTsvRaw
  .trim()
  .split('\n')
  .slice(1)
  .map(line => {
...
const rawNewData = `1	AGNEBY-TIASSA	Agboville	ADOU 	N'gbesso	Chef du village de Grand Morié 	0777880631	Nouveau
...
const newEntries: NewEntry[] = rawNewData
  .trim()
  .split('\n')
  .map(line => {
    const parts = line.split('\t').map(p => p.trim());
...
// Group old and new by (Region + Department)
const allDepartments = new Set<string>();
newEntries.forEach(n => allDepartments.add(`${normalize(n.region)}__${normalize(n.department)}`));
oldEntries.forEach(o => allDepartments.add(`${normalize(o.region)}__${normalize(o.department)}`));

interface DetailedDeptDiff {
  region: string;
  department: string;
  seats: {
    slotNum: number;
    ...
    newContact: string;
    newFonction: string;
    changeType: 'RECONDUIT' | 'REMPLACEMENT' | 'NOUVEAU_POURVU' | 'RESTE_A_POURVOIR';
    note: string;
  }[];
}

const deptDiffs: DetailedDeptDiff[] = [];

// Group new entries by region+dept
const newByDept = new Map<string, NewEntry[]>();
for (const n of newEntries) {
  const key = `${normalize(n.region)}__${normalize(n.department)}`;
  if (!newByDept.has(key)) newByDept.set(key, []);
  newByDept.get(key)!.push(n);
}

// Group old entries by region+dept
const oldByDept = new Map<string, OldEntry[]>();
for (const o of oldEntries) {
  const key = `${normalize(o.region)}__${normalize(o.department)}`;
  if (!oldByDept.has(key)) oldByDept.set(key, []);
  oldByDept.get(key)!.push(o);
}

let totalReconduits = 0;
let totalNouveaux = 0;
let totalVacants = 0;
let totalRemplacements = 0;
let totalNouveauxPourvus = 0;

for (const [key, nList] of newByDept.entries()) {
  const oList = oldByDept.get(key) || [];
  const region = nList[0]?.region || oList[0]?.region || '';
  const department = nList[0]?.department || oList[0]?.department || '';

  const usedOldIndices = new Set<number>();

  const deptResult: DetailedDeptDiff = {
    region,
    department,
    seats: []
  };

  // Match each new entry in dept with best old entry
  for (let idx = 0; idx < nList.length; idx++) {
    const n = nList[idx];

    if (!n.nomComplet) {
      deptResult.seats.push({
        slotNum: idx + 1,
        newNum: n.num,
        oldNum: oList[idx]?.num,
        oldName: oList[idx]?.nomPrenoms || '(A pourvoir)',
        oldObservation: oList[idx]?.observations || 'A pourvoir',
        newName: '(Non pourvu / Poste vide)',
        newProfile: 'A pourvoir',
        newContact: '-',
        newFonction: '-',
        changeType: 'RESTE_A_POURVOIR',
        note: 'Le poste reste vacant dans la nouvelle liste.',
      });
      totalVacants++;
      continue;
    }

    const normNName = normalize(n.nomComplet);
    const normNNom = normalize(n.nom);

    // Look for same person in old dept entries
    let matchedOldIdx = -1;
    for (let oIdx = 0; oIdx < oList.length; oIdx++) {
      if (usedOldIndices.has(oIdx)) continue;
      const o = oList[oIdx];
      const normOName = normalize(o.nomPrenoms);
      if (normOName === normNName || (normNNom && normOName.includes(normNNom) && normOName.split(' ')[0] === normNNom)) {
        matchedOldIdx = oIdx;
        break;
      }
    }
    ...
    if (matchedOldIdx !== -1) {
      usedOldIndices.add(matchedOldIdx);
      const matchedOld = oList[matchedOldIdx];
      deptResult.seats.push({
        slotNum: idx + 1,
        newNum: n.num,
        oldNum: matchedOld.num,
        oldName: matchedOld.nomPrenoms,
        oldObservation: matchedOld.observations,
        newName: n.nomComplet,
        newProfile: n.profile,
        newContact: n.contacts,
        newFonction: n.fonctionLocalite,
        changeType: 'RECONDUIT',
        note: `Titulaire reconduit avec mise à jour du contact (${n.contacts}) et fonction (${n.fonctionLocalite})`,
      });
      totalReconduits++;
    } else {
      // Find the first unused old slot in this department
      let availableOldIdx = -1;
      for (let oIdx = 0; oIdx < oList.length; oIdx++) {
        if (!usedOldIndices.has(oIdx)) {
          availableOldIdx = oIdx;
          break;
        }
      }

      const oldSlot = availableOldIdx !== -1 ? oList[availableOldIdx] : undefined;
      if (availableOldIdx !== -1) usedOldIndices.add(availableOldIdx);

      const wasVacantOrDeceased = oldSlot && (
        oldSlot.observations.toLowerCase().includes('décédé') ||
        oldSlot.observations.toLowerCase().includes('pourvoir') ||
        oldSlot.observations.toLowerCase().includes('directoire') ||
        oldSlot.observations.toLowerCase().includes('démissionnaire') ||
        oldSlot.nomPrenoms.toUpperCase().includes('NON POURVU')
      );

      const changeType = wasVacantOrDeceased ? 'NOUVEAU_POURVU' : 'REMPLACEMENT';
      if (changeType === 'NOUVEAU_POURVU') totalNouveauxPourvus++;
      else totalRemplacements++;
      totalNouveaux++;
      ...
      deptResult.seats.push({
        ...
        newNum: n.num,
        oldNum: oldSlot?.num,
        oldName: oldSlot?.nomPrenoms || '(Non renseigné)',
        oldObservation: oldSlot?.observations || '',
        newName: n.nomComplet,
        newProfile: n.profile,
        newContact: n.contacts,
        newFonction: n.fonctionLocalite,
        changeType,
        note: wasVacantOrDeceased
          ? `Nouveau titulaire pourvu en remplacement d'un poste vacant/décédé/nommé (${oldSlot?.observations})`
          : `Remplacement de "${oldSlot?.nomPrenoms || 'N/A'}" par un nouveau membre`,
      });
    }
  }

  deptDiffs.push(deptResult);
}

const statsSummary = {
  totalPostesFournis: newEntries.length,
  totalReconduits,
  totalNouveaux,
  totalNouveauxPourvusPourPostesVacants: totalNouveauxPourvus,
  totalRemplacementsDirects: totalRemplacements,
  totalPostesVacantsRestants: totalVacants,
};

fs.writeFileSync(
  path.resolve(process.cwd(), 'croisement_synthese.json'),
  JSON.stringify({ statsSummary, deptDiffs }, null, 2)
);

console.log('=== SYNTHÈSE COMPLÈTE DU CROISEMENT ===');
