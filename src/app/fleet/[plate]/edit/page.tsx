...
export default function FleetEditPage() {
    ...
    useEffect(() => {
        ...
        async function fetchVehicleData() {
            try {
                const vehicleData = await getVehicle(plate);
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="assignedTo" name="assignedTo" value={vehicle.assignedTo || ''} onChange={handleInputChange} />
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="maintenanceDue">Entretien Prévu</Label>
                        <Input id="maintenanceDue" name="maintenanceDue" type="date" value={vehicle.maintenanceDue || ''} onChange={handleInputChange} />
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="status">Statut</Label>
                         <Select value={vehicle.status || ''} onValueChange={(v) => handleSelectChange('status', v)}>
                            ...
                            <SelectContent>
                                {fleetStatuses.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                </CardContent>
