...
export function InstitutionalCover({
  ...
  return (
    <div className={cn(
        "flex flex-col bg-white p-12 print:p-16 break-after-page relative overflow-hidden",
        orientation === 'portrait' ? "min-h-[29.7cm]" : "min-h-[21cm]"
    )}>
      {/* Decorative Elements */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-5xl mt-8">
            {stats.map((stat, index) => (
              <div key={index} className="flex flex-col items-center gap-2 p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
                ...
                <span className="text-3xl font-black text-slate-900 tracking-tighter tabular-nums">{stat.value}</span>
